<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.xingquxiaozuName}}
					</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.xingquxiaozuContent"></rich-text>
				</view>
			</view>
			...
			<view class="time-content"
				...
				<view class="comoment-header"
					...
					<view v-if="jiaruxiaozuList>0 || detail.yonghuId == user.id"
						...
						<view class="cuIcon-add">添加打卡</view>
					</view>
				</view>
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop"
			v-if="jiaruxiaozuList<=0 && detail.yonghuId != user.id">
			...
			<view style="text-align: right;">
				...
				<button @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
					:style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">加入小组</button>
			</view>
		</view>
		</view>
		...
		methods: {
			...
			async onSubmit() {
				let data = {
					...
					xingquxiaozuId: this.detail.id
				}
				let res = await this.$api.save('jiarudexiaozu', data);
				this.jiaruxiaozuList = 1
			},
			// 获取详情
			async init() {
				let _this = this
				let res = await _this.$api.info('xingquxiaozu', _this.id);
				_this.detail = res.data;
				// 主图
				_this.swiperList = _this.detail.xingquxiaozuPhoto ? _this.detail.xingquxiaozuPhoto.split(",") : [];

				let params = {
					page: 1,
					limit: 9999,
					yonghuId: _this.user.id,
					xingquxiaozuId: _this.detail.id
				}
				let jiaruxiaozu = await _this.$api.list('jiarudexiaozu', params);
				if (jiaruxiaozu.data.list.length > 0) {
					_this.jiaruxiaozuList = jiaruxiaozu.data.list.length
				}
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('xingquxiaozuLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					xingquxiaozuId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../xingquxiaozuLiuyan/add-or-update?xingquxiaozuId=${this.id}`)
			},
		}
	}
</script>
