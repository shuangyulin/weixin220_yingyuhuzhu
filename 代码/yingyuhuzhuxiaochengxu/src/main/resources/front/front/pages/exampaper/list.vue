            ...
            <view class="list">
                ...
                <view @tap="onDetailTap(item.id)" v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
                    ...
                    <view class="listmr">
                        ...
                        <view class="colb f24 lh35">测试时长：{{item.exampaperDate}}分钟</view>
                        <view class="flex flex-align-center mt20" style="float: right;">
                            <text class="colb f25">
                                <button class="cu-btn bg-green shadow">点击测试</button>
                            </text>
                        </view>
                    </view>
                </view>
    ...
    export default {
        ...
        methods: {
            ...
            async onDetailTap(id) {
                ...
                let data = await this.$api.requestCondition('examrecord','saveExamrecord', value);
                this.$utils.msg('已创建测试记录');
                this.$utils.jump("./exam?id="+id+"&examrecordId="+data.id+"&paperUUID="+data.uuid)
            },
            // 搜索
            async search() {
                this.mescroll.num = 1
