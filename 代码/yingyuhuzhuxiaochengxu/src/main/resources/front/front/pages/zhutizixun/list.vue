<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.zhutizixunName" type="text" placeholder="资讯标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in zhutizixunTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
                ...
                class="uni-product"
                :style='{"borderRadius":"50rpx","backgroundColor":"#96b8c4"}' v-for="(product,index) in list" :key="index">
        <view class="uni-product-title"
              :style='{"fontSize":"26rpx","lineHeight":"40rpx","color":"#fff","textAlign":"center"}'>
            {{product.zhutizixunName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.zhutizixunPhoto?product.zhutizixunPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.zhutizixunNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.zhutizixunNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('zhutizixun','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('zhutizixun','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('zhutizixun','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let zhutizixunTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'zhutizixun_types'
		  	});
		  	this.zhutizixunTypesList = zhutizixunTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.zhutizixunName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.zhutizixunTypes = this.categoryName
                            }

                let res = await this.$api.list(`zhutizixun`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhutizixun', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.zhutizixunName) {
                    searchForm['zhutizixunName'] = this.searchForm.zhutizixunName
                }
				if (this.searchForm.zhutizixunName) {
					searchForm['zhutizixunName'] = this.searchForm.zhutizixunName
				}

                let res = await this.$api.list(`zhutizixun`, searchForm);
				// 如果是第一页数据置空
